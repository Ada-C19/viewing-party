...
# Edge case
# @pytest.mark.skip()
def test_adds_movie_already_in_watched():
    # Arrange
    movie = FANTASY_2
...
# Edge case
# @pytest.mark.skip()
def test_adds_movie_already_in_watchlist():
    # Arrange
    movie = FANTASY_2
...
def test_moves_movie_from_watchlist_to_empty_watched():
    ...
    assert len(updated_data["watched"]) == 1
    assert updated_data.get("watched")[0]["title"] == MOVIE_TITLE_1 
...
def test_moves_movie_from_watchlist_to_watched():
    ...
    assert len(updated_data["watchlist"]) == 1
    assert len(updated_data["watched"]) == 2
    assert HORROR_1 in updated_data.get("watched")
